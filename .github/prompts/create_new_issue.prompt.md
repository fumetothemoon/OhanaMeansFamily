...
2. Infer whether this is a bug report or feature request. Default to **Bug report** when the evidence describes broken or incorrect existing behavior; otherwise default to **Feature request**. Then pick a recommended label using `CONTRIBUTING.md` (Bug report: `bug`, `question`, `duplicate`, `invalid`, `wontfix`; Feature request: `enhancement`, `refactor`, `chore`, `documentation`).
3. Use the question picker to ask both questions in one interaction. Mark the inferred answers as recommended defaults:
   ...
   - **Label**: show only labels allowed by the selected issue type (sets above). If the user changes the issue type, update the label choices and ask again when the current label is not allowed.
